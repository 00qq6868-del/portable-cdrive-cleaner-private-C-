...
### 2026-04-29 加载期 DataGridView 前台隔离
- 范围：
  - `Forms/MainForm.cs`
- 已做：
  - 新增内容区加载遮罩 `_loadingShieldPanel`，扫描刷新期间将 `DataGridView` 从前台隐藏，只展示固定尺寸的刷新层和进度条。
  - 加载期仍允许后台准备候选数据，但不再让表格行、表头、滚动条参与前台重绘，直接规避加载中重复行、横向滚动条残影和表格脏像素。
  - 启动时预先 arm 加载视觉锁，避免首帧先显示表格再切换到加载态造成一次明显跳动。
  - 顶部任务摘要和加载层说明文字改为固定高度，避免进度百分比、已用时间、预计剩余时间每秒变化时触发 AutoSize 布局测量。
  - QA 状态新增 `LoadingShieldVisible`，用于确认加载期是否走遮罩模式。
- 验证：
  - `dotnet build .\PortableCDriveCleaner.csproj`：0 warning / 0 error。
  - 真实后台刷新安装版 QA 三轮通过，证据目录：`artifacts/icon-qa/2026-04-29_075203/`（本地原始截图，不提交公开仓库）。
  - 三轮启动时间：约 `2.83s / 2.20s / 2.19s`，DPI `168`，无残留进程。
  - QA 状态显示：`LoadingShieldVisible=true`、`ActiveGridDisplayed=false`、`ContentHeightRatio=0.849`、`TopChromeHeight=140`。
  - 人工复查 `00-loading-04.png`：加载中前台不再显示表格旧行、滚动条残影或横线堆叠。
- 状态：
